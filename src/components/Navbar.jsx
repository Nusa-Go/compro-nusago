  ...
  return (
    <nav className="fixed w-full z-50 px-4 py-4 md:px-10 transition-all duration-500">
      <div className={`max-w-[1200px] mx-auto flex items-center justify-between px-3 py-2 transition-all duration-500 rounded-full border border-slate-200/60 shadow-sm ${isScrolled ? 'bg-white/90 backdrop-blur-md shadow-lg' : 'bg-white/80 backdrop-blur-sm'
        ...
        {/* DESKTOP NAVIGATION - SEKARANG MENGGUNAKAN t() */}
        <div className="hidden md:flex items-center gap-8 text-[14px] font-bold text-slate-600 font-roboto">
          <a href="#home" onClick={(e) => scrollToSection(e, 'home')} className="hover:text-brand-red transition-colors">
            {t('nav.home')}
          </a>
          <a href="#about" onClick={(e) => scrollToSection(e, 'about')} className="hover:text-brand-red transition-colors">
            {t('nav.about')}
          </a>
          <a href="#services" onClick={(e) => scrollToSection(e, 'services')} className="hover:text-brand-red transition-colors">
            {t('nav.services')}
          </a>
          <a href="#features" onClick={(e) => scrollToSection(e, 'features')} className="hover:text-brand-red transition-colors">
            {t('nav.features')}
          </a>
          <a href="#partners" onClick={(e) => scrollToSection(e, 'partners')} className="hover:text-brand-red transition-colors">
            {t('nav.partners')}
          </a>
        </div>
        ...
        <div className="flex items-center gap-2 pr-1">
          <div className="hidden md:flex items-center gap-4 mr-2">
            ...
            >
              {/* Menampilkan nama bahasa yang aktif sesuai pilihan */}
          ...
          <button
            ...
            className="bg-[#0F172A] text-white px-6 py-2.5 rounded-full text-[13px] font-bold hover:bg-slate-800 transition-all active:scale-[0.98] font-roboto"
          >
            {t('nav.get_app')}
          </button>
      ...
      {/* MOBILE MENU PANEL - SEKARANG MENGGUNAKAN t() */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
